...
class OpenAIProvider: ObservableObject, AIProvider {
    ...
    func processText(systemPrompt: String? = "You are a helpful writing assistant.", userPrompt: String, images: [Data] = [], videos: [Data]? = nil) async throws -> String {
        // OpenAI's text completion endpoint doesn't support images or videos, so we ignore those parameters
        isProcessing = true
