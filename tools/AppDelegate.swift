...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    private func showPopup() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            NSLog("Pasteboard Types before image check: \(generalPasteboard.types ?? [])")

            let filteredImageTypes = supportedImageTypes.filter { type in
                type.rawValue.localizedCaseInsensitiveContains("pdf") == false
            }
            NSLog("Image types after PDF filter: \(filteredImageTypes)")

            for type in filteredImageTypes {
                if let data = generalPasteboard.data(forType: type) {
                    foundImages.append(data)
                    NSLog("Detected image type: \(type)")
                }
            }
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
                ...
                for type in filteredImageTypes {
                    if let data = generalPasteboard.data(forType: type) {
                        if !foundImages.contains(data) {
                            foundImages.append(data)
                            NSLog("Detected new image type after copy: \(type)")
                        }
                    }
                }
    ...
    @objc func handleSelectedText(_ pboard: NSPasteboard, userData: String, error: AutoreleasingUnsafeMutablePointer<NSString>) {
        if let frontmostApp = NSWorkspace.shared.frontmostApplication {
            ...
            NSLog("Pasteboard Types (Service) before image check: \(pboard.types ?? [])")

            let filteredImageTypes = supportedImageTypes.filter { type in
                type.rawValue.localizedCaseInsensitiveContains("pdf") == false
            }
            NSLog("Image types after PDF filter (Service Menu): \(filteredImageTypes)")

            for type in filteredImageTypes {
                if let data = pboard.data(forType: type) {
                    foundImages.append(data)
                    NSLog("Detected image type (Service): \(type)")
                }
            }
            ...
            appState.selectedImages = foundImages
