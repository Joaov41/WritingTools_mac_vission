...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    // ─── NEW: Handle incoming URL requests (if launched via custom URL) ─────────
    func application(_ application: NSApplication, open urls: [URL]) {
        for url in urls {
            if url.scheme == "writingtools", url.host == "processSharedText" {
                // Instead of calling a missing checkForSharedContent, call processURLFromClipboard
    ...
    private func showPopup() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if let pdfData = generalPasteboard.readPDF() {
                ...
                // Clear the pasteboard so that PDF data is not retained
            ...
            if let videoData = generalPasteboard.readVideo() {
                ...
                // Clear the pasteboard so that video data is not retained
                generalPasteboard.clearContents()
            ...
            // Get initial pasteboard content (for text and images)
...
extension AppDelegate {
    func setupPDFDragAndDrop() {
        ...
        let fileTypes = [kUTTypePDF] as CFArray
