...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    private func showPopup() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let supportedTypes: [NSPasteboard.PasteboardType] = [.tiff, .png, .pdf]
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
                ...
                // Check for new images after copy
                for type in supportedTypes {
                    if let data = pasteboard.data(forType: type) {
    ...
    @objc func handleSelectedText(_ pboard: NSPasteboard, userData: String, error: AutoreleasingUnsafeMutablePointer<NSString>) {
        ...
        // Check for images
        var foundImages: [Data] = []
        let imageTypes: [NSPasteboard.PasteboardType] = [.tiff, .png, .pdf]
